<template>
  <div v-if="currentUser.type == 'waiter'">
    <br>
    <a class="btn btn-info" @click.prevent="registerMeal()">
      <i class="fas fa-plus-circle"></i> Register Meal
    </a>
    <br>
    <br>
    <registerMeal
      :registeringMeal="registeringMeal"
      :tables="tables"
      @save-Meal="saveMeal"
      @cancel-Meal="cancelMeal"
    ></registerMeal>
    <br>
    <br>
    <registerOrder
    ...
    <table class="table table-striped">
      <tbody
        v-for="meal in meals"
        :key="meal.id"
        v-if="meal.responsible_waiter_id == currentUser.id && meal.state == 'active'"
      >
        <tr>
          <td>Meal: {{ meal.id }}</td>
          ...
          <td>
            <a class="btn btn-sm btn-primary" @click.prevent="registerOrder(meal)">Register Orders</a>
            <a class="btn btn-sm btn-primary" @click.prevent="terminate(meal)">Terminate</a>
          </td>
        </tr>
        <tr v-for="order in orders" :key="order.id " v-if="meal.id == order.meal_id">
          ...
          <td>
            <a
              ...
              @click.prevent="setOrderState(order)"
              v-if="order.state == 'prepared'"
            >Deliver</a>
            <a
              class="btn btn-sm btn-danger"
              @click.prevent="cancelOrder(order)"
              v-if=" plusFiveSeconds(order.start) >= currentDate"
            >Cancel</a>
          </td>
        </tr>
      </tbody>
...
module.exports = {
  ...
  methods: {
    ...
    setOrderState(order) {
      ...
      meal.total_price_preview += axios
        .put(
          "api/meals/" + meal.id + "/" + this.items[order.item_id - 1].price,
          meal
        )
        .then(response => {
          this.getMeals();
    ...
    terminate: function(meal) {
      ...
      if (response) {
        ...
        axios
          .post("api/invoices/register", this.invoice)
          .then(response => {
            for (let order of this.orders) {
              ...
              if (meal.id == order.meal_id) {
                ...
                for (let element of this.orders) {
                  if (
                    invoiceitem.item_id == element.item_id &&
                    meal.id == element.meal_id
                  ) {
                    invoiceitem.quantity += 1;
                    invoiceitem.unit_price = this.items[
                      order.item_id - 1
                    ].price;
                  }
                }

                invoiceitem.sub_total_price =
